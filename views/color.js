...
class Color$1 {
	...
	static inGamut (space, coords, {epsilon = Îµ} = {}) {
		space = Color$1.space(space);
...
Color$1.defineSpace({
	...
	white: Color$1.whites.D50,
	Îµ: 216/24389,  // 6^3/29^3 == (24/116)^3
	Îµ3: 24/116,
	Îº: 24389/27,   // 29^3/3^3
	// Îº * Îµ  = 2^3 = 8
	fromXYZ(XYZ) {
		// Convert D50-adapted XYX to Lab
		//  CIE 15.3:2004 section 8.2.1.1
		const {Îº, Îµ, white} = this;
...
Color$1.defineSpace({
	...
	cssid: "rec2100-pq",
	name: "REC.2100-PQ",
	Yw: 203,	// absolute luminance of media white, cd/mÂ²
	n: 2610 / (2 ** 14),
	ninv: (2 ** 14) / 2610,
	m: 2523 / (2 ** 5),
	minv: (2 ** 5) / 2523,
...
Color$1.defineSpace({
	...
	toLinear(RGB) {
		...
		return RGB.map(function (val) {
			if (val <= low) {
				return (2 ** ((val * 17.52) - 9.72) - Îµ) * 2; // 0 for low or below
			}
			else if (val < high) {
...
Color$1.prototype.deltaEJz = function (sample) {
	...
	let Î”h = Hz1 - Hz2;
	let Î”H = 2 * Math.sqrt(Cz1 * Cz2) * Math.sin(Î”h * Math.PI / 180);
...
Color$1.prototype.deltaEOK = function (sample, deltas = {}) {
	...
	let H2 = (Î”a ** 2) + (Î”b ** 2) - (deltas.Î”C ** 2);
	...
	deltas.Î”H = Math.sqrt(H2);

	// Calculate the deltaE, term by term as root sume of squares
	let dE = deltas.Î”L ** 2 + deltas.Î”C ** 2 + H2;
	return Math.sqrt(dE);
};
